...
check_file()
{
    ui_print " "
    ui_print " --- 检查必要文件是否存在 ---"
    if [[ ! -f "/sys/class/oplus_chg/battery/status" || ! -f "/sys/class/oplus_chg/battery/current_now" || ! -f "/sys/class/oplus_chg/battery/capacity" ]]; then
        ui_print " ！缺少必要文件，不支持此手机，安装失败！"
        ui_print " "
        rm -rf ${MODPATH}
        exit 1
    fi
    if [[ -f /sys/class/oplus_chg/battery/chip_soc ]]; then
        ui_print " - 检测到/sys/class/oplus_chg/battery/chip_soc文件存在，使用此文件作为真实电量文件"
        sed -i 's|^capacity_file=.*|capacity_file=/sys/class/oplus_chg/battery/chip_soc|g' ${TMPDIR}/service.sh
    elif [[ -f /sys/class/oplus_chg/battery/real_capacity ]]; then
